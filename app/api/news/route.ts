...
export async function POST(req: NextRequest) {
  ...
  const toAnalyze = enrichedArticles.filter((article) => {
    const titleWords = new Set<string>(article.title.toLowerCase().split(/\W+/).filter((w: string) => w.length > 3))
    const isDuplicate = seenTitles.some((seen) => {
      const seenWords = new Set<string>(seen.toLowerCase().split(/\W+/).filter((w: string) => w.length > 3))
